import SwiftUI

enum AppColors {
    // MARK: - Color Definitions
    
    static func accent(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "7C3AED") : Color(hex: "4318D1")
    }
    
    static func primary(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "7C3AED") : Color(hex: "4318D1")
    }
    
    static func secondary(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "8B5CF6") : Color(hex: "6D28D9")
    }
    
    static func background(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "151823") : Color(hex: "F7F8FC")
    }
    
    static func surface(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "1E293B") : Color(hex: "FFFFFF")
    }
    
    static func surfaceVariant(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "1E293B") : Color(hex: "F1F5F9")
    }
    
    static func textPrimary(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "F8FAFC") : Color(hex: "1E293B")
    }
    
    static func textSecondary(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "E2E8F0") : Color(hex: "475569")
    }
    
    static func textTertiary(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "CBD5E1") : Color(hex: "64748B")
    }
    
    static func error(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "EF4444") : Color(hex: "DC2626")
    }
    
    static func warning(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "F59E0B") : Color(hex: "D97706")
    }
    
    static func info(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "3B82F6") : Color(hex: "2563EB")
    }
    
    static func success(for colorScheme: ColorScheme) -> Color {
        colorScheme == .dark ? Color(hex: "10B981") : Color(hex: "059669")
    }
}

// MARK: - Color Extension for Hex Support
extension Color {
    init(hex: String) {
        let hex = hex.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int: UInt64 = 0
        Scanner(string: hex).scanHexInt64(&int)
        let a, r, g, b: UInt64
        switch hex.count {
        case 3: // RGB (12-bit)
            (a, r, g, b) = (255, (int >> 8) * 17, (int >> 4 & 0xF) * 17, (int & 0xF) * 17)
        case 6: // RGB (24-bit)
            (a, r, g, b) = (255, int >> 16, int >> 8 & 0xFF, int & 0xFF)
        case 8: // ARGB (32-bit)
            (a, r, g, b) = (int >> 24, int >> 16 & 0xFF, int >> 8 & 0xFF, int & 0xFF)
        default:
            (a, r, g, b) = (1, 1, 1, 0)
        }
        self.init(
            .sRGB,
            red: Double(r) / 255,
            green: Double(g) / 255,
            blue:  Double(b) / 255,
            opacity: Double(a) / 255
        )
    }
} 